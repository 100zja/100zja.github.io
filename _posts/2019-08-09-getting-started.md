...
* **환경변수를 이용하는 방법** : 환경변수 - 응용프로그램에서 참조하여 사용할 수 있음 --> 공격자는 환경 변수를 만든 뒤 이 안에 쉘 코드를 넣고 취약한 프로그램에 환경변수의 address를 retrun address에 넣음 --> 쉘 코드 실행  
...
스텍 영역의 코드를 실행하지 못하게 하는 non-executable stack 보호 기법이나 일부 IDS에서 네트워크를 통해 쉘 코드가 유입되는 것을 차단하는 보호 기법을 뚫기 위한 방법으로 제안됨  
  - **non-executable stack** : 스텍 영역에 있는 코드를 실행하지 못하게 하는 것, CPU 레벨에서의 보호 방법이기도 하고 운영체제 영역에서의 보호 방법이기도 함, EIP 레지스터에 stack segment 영역의 주소가 들어가면 에러 메시지 출력, 실행 종료 or 에러 메시지 없이 실행 멈춤  

  - **IDS** : 일부 IDS는 네트워크 인터페이스에서 수신되는 데이터에 쉘 코드가 포함되어 있거나 비정상적으로 많은 양의 NOP가 포함되어 있다면 침입으로 간주해 네트워크 연결을 종료함
===> 이러한 공격기법 등장으로 고전 기법 먹혀들지 X  

* **beist's execl 방법** : root의 권한을 얻어오기 위해서는 excel() 함수를 사용할 수 있음  
buffer overflow 취약점을 가진 프로그램을 return-into-libc 기법으로 overflow 시켜 공격  
==> non-executable stack 보호 기법을 회피하여 특정 명령을 수행할 수 있음, 스텍 영역에 쉘 코드를 집어넣지 않고 실행 가능, 버퍼 크기에 제약을 받지 않아 쉘 코드를 넣을 충분한 버퍼를 가지지 않은 취약 프로그램도 충분히 공격 가능