...
* **ASLR** :  
Address Space Layout Randomization, 메모리상의 공격을 어렵게 하기 위해 스택이나 힙, 라이브러리 등의 주소를 랜덤으로 프로세스 주소 공간이 배치함으로써 실행할 때마다 데이터의 주소가 바뀌게 하는 기법
...
* **Eggshell 기법** :  
  ...
  1. 동작 범위에 따른 환경 변수
    ...
    - **시스템 전체 환경 변수** : 해당 시스템에 존재하는 모든 사용자가 사용할 수 있는 환경 변수, 시스템 전원이 켜져 있고 모든 사용자가 로컬 또는 원격으로 로그인 할 때마다 로드 됨  
  - export [환경변수명]=필요한 정보  
    환경변수 입력시 경로임을 표시해주는 '$' 을 앞에 붙여줘야 함  
    쉘코드의 경우 (python -c 'print "쉘코드"') 로 입력해줘야 함
